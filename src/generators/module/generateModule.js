...
export const generateModule = (opts) => {
  const { name, options } = opts;
  if (!isString(name)) throw Error();
  const lowerCaseName = name.toLowerCase();
  const template = `import { Module } from '@nestjs/common';
${options.service ? `import { ${name}Service } from './${lowerCaseName}.service';` : ""}
${options.controller ? `import { ${name}Controller } from './${lowerCaseName}.controller';` : ""}
import { SequelizeModule } from '@nestjs/sequelize';
${options.entity ? `import { ${name} } from './entities/${lowerCaseName}.entity';` : ""}
${options.entity ? `import { ${name}Repository } from './${lowerCaseName}.repository';` : ""}
