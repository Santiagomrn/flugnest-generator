...
export const generateFactories = (opts) => {
...
export const generateControllerSpec = (opts) => {
  const { name } = opts;
  if (!isString(name)) throw Error();
  const lowerCaseName = name.toLowerCase();
...
export const generateControllerE2E = (opts) => {
  const { name, belongsUser = false } = opts;
  if (!isString(name)) throw Error();
  const lowerCaseName = name.toLowerCase();
  const template = `import { Test, TestingModule } from '@nestjs/testing';
...
${belongsUser ? "import { User } from '@modules/user/entities/user.entity';" : ""}
