...
export class QueueController {
  ...
  constructor(@Inject("SB_CLIENT") private readonly sbClient: AzureServiceBusClientProxy) {}
  ...
  handleQueueMessage(@Payload() data: ServiceBusMessage, @Ctx() context: AzureServiceBusContext) {
  ...
  handleTopicMessage(@Payload() data: ServiceBusMessage, @Ctx() context: AzureServiceBusContext) {
    const serviceBusReceiver: ServiceBusReceiver = context.getArgs()[0];
    this.logger.info(data.body);
  }
}
